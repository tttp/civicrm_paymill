...
{if $form.credit_card_number or $form.bank_account_number}
    ...
    var PAYMILL_PUBLIC_KEY = '{$paymentProcessor.password}';
     {literal}
        cj(function() {
         ...
         cj(document).ready(function() {
            ...
            cj('form.paymill-payment-form input.form-submit').attr("disabled", "disabled");
                if(cj(this).find("#priceset input[type='radio']:checked").data('amount') == 0) {
                return true;
            }
            
            //window.alert("Delam token");
            window.alert(cj('#amount_level').val());
        ...
        if (!paymill.validateExpiry(
          ...
          window.alert("Napačen datum");
            ...
            paymill.createToken({
                  ...
                  cvc: cj('#cvv2').val(),
                  cardholder: cj('#billing_first_name').val() + ' ' + cj('#billing_last_name').val(),
                  currency: 'EUR', 
                  amount_int:  '2400'
      ...
      // Token
        ...
        //window.alert(result.token);
                    cj("input#paymill-token").val(result.token);
                    cj("form.paymill-payment-form").get(0).submit();
     }
